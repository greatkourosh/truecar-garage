...
def index(request, html_template="index"):
...
def load_car(car_brand="toyota", car_model="rav4", number_of_cars=10):
    ...
    car_ads_data = get_car_ad_details(car_brand, car_model, number_of_ads=number_of_cars)
    filtered_number_of_car = number_of_cars if number_of_cars > 0 else 10
    print(f"filtered_number_of_car = {filtered_number_of_car}")
    for car_ad_data in car_ads_data[:(filtered_number_of_car)]:
...
def load_car_view(request, car_brand="toyota", car_model="rav4", html_template="load_car", number_of_cars=10):
    # number_of_cars=10
    car_ads_data = load_car(car_brand, car_model, number_of_cars)
    context = {
        "cars": car_ads_data,
        "page_name": html_template,
        'debug_flag': debug_flag,
...
def select_car_old(request, html_template="select_car_old"):
    # car = generate_or_return_car('5UXTY5C0XM9H22657')
    # car_brands = Car.Brand.choices
    # print(car_brands)
    # context = {
    ...
    if request.method == 'POST':
        ...
        number_of_cars = int(request.POST.get('number_of_cars'))
        if debug_flag:
            print(car_brand)
            print(car_model)
        ...
        load_car_view(request, car_brand=car_brand.replace(' ', '-').lower(), car_model=car_model.replace(' ', '-').lower(), number_of_cars=number_of_cars)
    return render(request, f"garage/{html_template}.html", context)

class SelectCar(FormView):
    template_name = f"garage/select_car.html"
    # context ={
    ...
    def form_valid(self, form=form_class):
        ...
        car_brand = car_brand=form.cleaned_data["car_brand"].replace(' ', '-').lower()
        car_model=form.cleaned_data["car_model"].replace(' ', '-').lower()
        car_ads_data = load_car(car_brand=car_brand, car_model=car_model, number_of_cars=number_of_cars)
        # index(request)
        return super(SelectCar, self).form_valid(form)
...
def TestAdminPage(request, html_template="test_admin_page"):
    context ={
        "page_name": html_template,
        'debug_flag': debug_flag,
    }
